...
# donne toutes les informations pour un kebab repéré par son 'emplacement'
def infoskebab(emplacement):
	infos={} 
	
	# nom
	infos['nom']=driver.find_element_by_id(emplacement).find_element_by_class_name('title').text
	
	# adresse
	infos['adresse']=driver.find_element_by_id(emplacement).find_element_by_class_name('txt').text
	
	# note
	infos['note']=driver.find_element_by_id(emplacement).find_element_by_class_name('bg_note').get_attribute('innerHTML')
...
# chope les informations de tous les kebabs d'un arrondissement ou d'une ville et les ajoute à la liste paris 
def scrapepage(paris):
	...
	# ON GENERE LES EMPLACEMENTS DE TOUS LES KEBABS - ceux-ci sont repérés par lieu_xx
	listeemplacements=[]
	...
	if(nombrekebabs>=20):
		...
		# on trouve le lien 
		liens=driver.find_element_by_id('col_gauche').find_element_by_class_name('filter_right2').find_elements_by_tag_name('a')
		longueur=len(liens)
...
def scrapeville(url):
	...
	listeliens=driver.find_elements_by_class_name('fleche') # liste des villes/arrondissements du departement
